...
class SubtitleAlignRequest(StrictModel):
    method: Literal["ctc", "ctc_asr_fallback"] = "ctc"
    expected_revision: int = Field(ge=0)
...
class GenerationSegmentSelector(StrictModel):
...
class GenerationSplitBoundary(StrictModel):
...
class GenerationTopologyEdit(StrictModel):
...
class GenerationPlanBatchRequest(StrictModel):
