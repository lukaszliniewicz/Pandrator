...
_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "stt": {
        "stt_engine": "Selects the speech-to-text engine: local CrispASR Whisper, Parakeet, MOSS, or Qwen3 ASR, or the supported Azure MAI-Transcribe-1.5 cloud profile. Qwen3 is a recognizer; its word timestamps come from the separate Qwen3 forced aligner where the source language is supported.",
        ...
        "qwen_asr_chunk_seconds": "Sets the maximum Qwen3 ASR chunk length in seconds. Zero uses the bounded 30-second default; longer windows require a proportional decode token budget.",
        ...
        "qwen_asr_chunk_mode": "Selects Qwen3 chunking in CrispASR. Auto follows the voice activity detection setting (enabled by default); vad always uses Silero; fixed and legacy none use bounded fixed windows without VAD.",
        ...
        "qwen_asr_clamp_timestamps": "Legacy compatibility value; CrispASR supplies native word timestamps, which are validated without repairing or fabricating spans.",
        "stt_compute_device": "Selects the non-CPU local accelerator index passed to CrispASR sessions; it is ignored for auto/CPU execution and for remote cloud STT.",
        ...
        "moss_ctc_alignment_enabled": "Enables local forced alignment that attaches timed text units to native MOSS speaker turns. Qwen3 is non-autoregressive, not CTC; the setting name is retained for compatibility.",
        "moss_ctc_aligner_model": "Selects the forced aligner for native MOSS turns. Auto uses Qwen3 for Japanese/Chinese/Korean/Cantonese and Canary for other supported languages. Qwen3 requires audio.cpp and caches a verified 1.13 GB model once; failed turns retain native timing.",
        ...
        "crispasr_vad_enabled": "Enables VAD for local CrispASR Whisper, Parakeet and Qwen3 transcription; MOSS uses moss_vad_enabled instead, and Azure cloud STT has its own remote chunking behavior.",
        ...
        "crispasr_vad_max_speech_seconds": "Sets the maximum local CrispASR VAD speech region before splitting. Qwen3 additionally caps it at its chunk length to stay within the decode token budget.",
        ...
        "caption_alignment_ctc_model": "Selects the caption forced aligner. Auto uses Qwen3 through audio.cpp for Japanese/Chinese/Korean/Cantonese and Canary CTC otherwise. Explicit Qwen3 supports 11 languages and uses a verified 1.13 GB cache. Exact transcript/audio pairs are aligned locally in bounded groups; no ASR model is needed.",
    ...
    "tts": {
        ...
        "speech_block_continuation_threshold_ms": "Sets the outer pause allowance for unfinished same-speaker phrases. Above the normal silence limit, only a non-clausal hesitation may be bridged, within a cumulative pause budget and a 30-second phrase window. Zero disables continuation across positive gaps.",
        "speech_block_max_internal_gap_ms": "Sets the normal silence limit inside a speech block. Natural clause and sentence pauses retain this guard; an unfinished same-speaker phrase may use the larger continuation allowance rather than split mid-phrase. Zero retains a strict no-positive-gap limit.",
    ...
    "output": {
        ...
        "video_tail_extension_policy": "When speech extends beyond a video, ask stops before rendering and reports the extra duration; extend freezes the last frame for the full required duration, with no limit. Existing speech timing is preserved. Video must be reencoded.",
...
_METADATA: dict[str, dict[str, dict[str, object]]] = {
    ...
    "stt": {
        ...
        "qwen_asr_chunk_seconds": {
            ...
            "maximum": 120,
            "unit": "seconds",
            "applicability": "Qwen3 ASR only; 0 selects bounded 30-second chunks.",
        },
        "qwen_asr_max_tokens": {
            "minimum": 32,
            "maximum": 4096,
        ...
        "caption_alignment_ctc_model": {
            "applicability": "Media-edit caption forced alignment only.",
        },
    ...
    "output": {
        ...
        "video_tail_extension_policy": {"choices": ["ask", "extend"]},
