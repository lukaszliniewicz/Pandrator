...
Model capability filters use `audio_cpp_model_metadata.json`, generated by
`python scripts/generate_audio_cpp_model_metadata.py` from the application's
canonical catalogue. Run the same command with `--check` to detect drift. The
standalone package includes this projection and has no runtime dependency on
the application. Family grouping changes presentation only; installation always
uses the original package IDs. Chatterbox and FireRed variants share umbrella
families while retaining their individual capabilities.
