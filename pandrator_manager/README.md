...
The current release is 0.9.1. For most users, the easiest installation is the
Windows executable or Linux AppImage on the
[Pandrator 0.6.1 release page](https://github.com/lukaszliniewicz/Pandrator/releases/tag/v.0.6.1).
...
**Unknown publisher** or SmartScreen warning. Verify its release checksum when
you download it.
...
Use `pandrator-manager[tray]` instead if you want the optional desktop tray.
For a downloaded wheel, replace the package name with its local path.
...
problems or suggestions. Include the operating system, workspace layout,
action you attempted, and logs with secrets removed.
