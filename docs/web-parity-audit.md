...
| Reusable sources | Source Library and session Sources | Partial | Attachment history, current selection, detach, reference-aware trash/restore and rename are implemented; external-reference policy and doctor/repair actions remain |
...
| Subtitle review | Text & Subtitles comparison editor | Partial | Legacy temporal alignment, lineage merges/splits, save conflict, and reviewed-revision invalidation have repository evidence; timing-conflict and audio-scrubbing browser qualification remain |
| Audiobook generation | Session text/voice/output tabs and generation drawer | Partial | Typed chapter boundaries, cover upload/preview, metadata, M4B chapters, and container tags have repository evidence; long-document and pause/restart runtime qualification remain |
...
1. Complete the remaining video audio/subtitle export matrix.
2. Qualify generation pause/restart recovery against migrated Qt and web-preview fixtures.
3. Finish source doctor/repair UX and external-reference policy.
