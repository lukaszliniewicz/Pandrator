<script lang="ts">
  import { ChevronDown, RotateCcw, Save } from '@lucide/svelte';
  import { api } from './api';
  let {sessionId,section,title,description=''}:{sessionId:string;section:string;title:string;description?:string}=$props();
  let payload=$state<any>(null);let override=$state<Record<string,any>>({});let advanced=$state(false);let saving=$state(false);let message=$state('');
...
<section class="surface rounded-2xl p-5">
  <div class="flex flex-wrap items-start justify-between gap-4">
    <div><div class="eyebrow">{section.replaceAll('_',' ')}</div><h2 class="mt-1 text-xl font-semibold">{title}</h2>{#if description}<p class="muted mt-2 max-w-2xl text-sm">{description}</p>{/if}</div>
    <div class="flex gap-2"><button onclick={reset} class="tool"><RotateCcw size={14}/> Inherit defaults</button><button onclick={save} disabled={saving} class="tool bg-[var(--accent)] text-white"><Save size={14}/> {saving?'Saving…':'Save'}</button></div>
  </div>
  {#if payload}
