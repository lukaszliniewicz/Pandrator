<script lang="ts">
  ...
  const entries = $derived(
    Object.entries(payload?.effective ?? {}).sort(([left], [right]) => {
      ...
      const li = order.indexOf(left),
        ri = order.indexOf(right);
      return (
        (li < 0 ? 999 : li) - (ri < 0 ? 999 : ri) || left.localeCompare(right)
      );
    })
  );
  ...
  const sectionName = (value: string) =>
    (({ tts: 'TTS', stt: 'STT', rvc: 'RVC' }) as Record<string, string>)[
      value
    ] ?? value.replaceAll('_', ' ');
  const applicable = $derived(
    entries.filter(([key]) => {
      ...
      if (
        ...
        return service.includes('xtts');
      if (key.startsWith('openai_audio_'))
        return (
  ...
  const visible = $derived(
    applicable.filter(
      ([key]) => advanced || (common[section] ?? []).includes(key)
    )
  );
  ...
  const researchEnabled = $derived(
    Boolean(
      value(
        'web_research_enabled',
        payload?.effective?.web_research_enabled ?? false
      )
    )
  );
  ...
  const llmText = $derived(
    applicable
      ...
      .filter(([key]) => {
        if (
          ['llm_tts_optimization', 'llm_tts_document_optimization'].includes(
            key
          )
        )
          return true;
        if (
          !value(
            'llm_tts_optimization',
            payload?.effective?.llm_tts_optimization
          ) &&
          !value(
            'llm_tts_document_optimization',
            payload?.effective?.llm_tts_document_optimization
          )
        )
          return false;
        ...
        if (
          ...
          return false;
        if (key === 'speech_plan_save_proposals' && planningMode === 'legacy')
          return false;
        const divided = Boolean(
          value('llm_multi_stage', payload?.effective?.llm_multi_stage)
        );
    ...
    try {
      if (section === 'tts')
        override = Object.fromEntries(
          Object.entries(override).filter(([key]) => !providerSetting(key))
        );
      payload = await sessionApi.saveSettings(
        sessionId,
        section,
        payload!.revision,
        override
  ...
  {#if payload}
    {#if section === 'text'}
        ...
        >
          ...
          <div class="mt-4 grid gap-4">
            {#each llmText as [key, fallback]}<div>
                ...
                /><span
                  class="mt-1 block text-[.65rem] text-[var(--accent)]"
                  class:muted={!Object.prototype.hasOwnProperty.call(
                    override,
                    key
                  )}
                  >{Object.prototype.hasOwnProperty.call(override, key)
          ...
          {#if researchEnabled}<p class="muted mt-3 text-[.68rem]">
      ...
      <button
        ...
        ><ChevronDown class={advanced ? 'rotate-180' : ''} size={14} />{advanced
          ? 'Hide'
          : 'Show'} advanced deterministic settings</button
      >
    {:else if stageResearchSection}
      ...
      <section class:enabled={researchEnabled} class="research-card mt-5">
        <div class="flex flex-wrap items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            <div class="text-sm font-semibold">Bounded web research</div>
            <p class="muted mt-1 max-w-3xl text-xs leading-relaxed">
              Before {section}, the selected LLM may ask Jina to search and
              extract a small number of pages. Pandrator keeps a source ledger,
              rejects unsourced evidence, and treats page content as untrusted.
            </p>
          </div>
          <a href="/providers?tab=credentials" class="tool"
            ><ExternalLink size={14} /> Jina API key</a
          >
        </div>
        <div class="mt-4 max-w-sm">
          <SettingField
            {section}
            keyName="web_research_enabled"
            value={researchEnabled}
            onchange={(next) => set('web_research_enabled', next)}
            compact
          /><span
            class="mt-1 block text-[.65rem] text-[var(--accent)]"
            class:muted={!Object.prototype.hasOwnProperty.call(
        ...
        {#if deepLResearchConflict}<div
            class="mt-4 flex gap-2 rounded-xl bg-amber-500/10 p-3 text-xs text-amber-700"
          >
            <TriangleAlert class="mt-0.5 shrink-0" size={14} />Web research can
            augment the LLM translation backend only. Choose LLM translation or
            turn research off before saving.
          </div>{/if}
        {#if researchEnabled}
          <div
            class="mt-5 grid gap-4 border-t border-[var(--line)] pt-5 sm:grid-cols-2 xl:grid-cols-3"
          >
      ...
      {#if applicable.length > (common[section]?.length ?? 0)}<button
          ...
          ><ChevronDown
            ...
            size={14}
          />{advanced ? 'Hide' : 'Show'} advanced settings and research budgets</button
        >{/if}
    {:else}
      <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      ...
      {#if applicable.length > (common[section]?.length ?? 0)}<button
          ...
          ><ChevronDown
            class={advanced ? 'rotate-180' : ''}
            size={14}
          />{advanced ? 'Hide' : 'Show'} advanced settings</button
        >{/if}
    {/if}
  {/if}
  {#if message}<p
