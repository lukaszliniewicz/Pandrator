<script lang="ts">
  ...
  let sourceMode = $state<'upload' | 'paste' | 'url' | 'reuse' | 'later'>(
    'upload'
  );
  ...
  const duplicateSession = $derived(
    appState.sessions.find(
      ...
      (duplicateFromServer?.name.trim().toLocaleLowerCase() ===
  ...
  const isSrt = $derived(
    (
      sourceFile?.name ??
      reusableSources.find((item) => item.id === sourceAssetId)?.display_name ??
      ''
    )
      .toLowerCase()
      .endsWith('.srt')
  ...
  const pipeline = $derived([
    ...
    kind === 'subtitles'
      ? subtitleExport === 'text'
        ? 'Export text'
        : `Export ${subtitleExport.toUpperCase()}`
      : 'Export'
  ]);
  ...
  async function create(overwrite = false) {
    ...
    error = '';
    progress = 0.05;
    progressDetail = 'Creating workspace';
    try {
        ...
        export: {
          ...
          subtitle_mode:
            kind === 'audiobook' || kind === 'subtitles'
              ? 'none'
              : subtitleMode,
          subtitle_format: subtitleExport === 'text' ? 'srt' : subtitleExport,
          subtitles: translate
            ? keepSourceSubtitles
              ? 'dual'
              : 'translation'
      ...
      progress = 0.3;
      progressDetail = 'Saving workspace settings';
      ...
      const updateSettings = async (
        ...
        await sessionApi.saveSettings(session.id, section, stored.revision, {
          ...stored.override,
          ...next
        });
        completedSettings += 1;
        progress = 0.3 + 0.35 * (completedSettings / 4);
        progressDetail = `Saved workspace setting ${completedSettings} of 4`;
      };
      const speechLanguage = translate
        ? targetLanguage
        : sourceLanguage === 'auto'
          ? 'en'
          : sourceLanguage;
      const subtitleSelection = translate
        ? keepSourceSubtitles
          ? 'dual'
          : 'translation'
        : 'source';
      const outputSettings =
        kind === 'audiobook'
          ...
          : kind === 'subtitles'
            ? {
                export_mode: subtitleExport === 'text' ? 'text' : 'subtitles',
                subtitle_format:
                  subtitleExport === 'text' ? 'srt' : subtitleExport,
      ...
      let file =
        sourceMode === 'paste' && pastedText.trim()
          ...
          : sourceMode === 'upload'
            ...
            : null;
      if (file) {
        progressDetail = 'Uploading source';
        await uploadManagedFile(
          file,
          session.id,
          (value) => (progress = 0.65 + Math.max(0, Math.min(1, value)) * 0.3)
        );
      } else if (sourceMode === 'url' && sourceUrl.trim()) {
        progress = 0.8;
        progressDetail = 'Queueing source download';
        await sessionApi.downloadSourceUrl(session.id, sourceUrl.trim());
      ...
      if (caught instanceof ApiError && caught.code === 'duplicate_session') {
        duplicateFromServer =
          (caught.details as { existing_session?: SessionRecord } | null)
            ?.existing_session ?? null;
        error = '';
      } else error = errorMessage(caught);
    } finally {
      creating = false;
    }
  }
...
>
  ...
  <section
    ...
    class="surface max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] p-6 sm:p-9"
    role="dialog"
    aria-modal="true"
  ...
  >
    <header class="flex items-start justify-between gap-4">
      ...
      <button onclick={onclose} class="rounded-xl p-2" aria-label="Close"
        ><X size={20} /></button
      >
    </header>
    {#if error}<div
        class="mt-5 rounded-xl border border-red-400/40 bg-red-500/10 p-3 text-sm"
      >
        {error}
      </div>{/if}
    {#if step === 1}
      <div class="mt-7 grid gap-4 md:grid-cols-3">
        <button onclick={() => chooseKind('audiobook')} class="choice"
          ...
          ></button
        ><button onclick={() => chooseKind('subtitles')} class="choice"
          ><Captions size={25} /><strong>Create subtitles</strong><span
            >Transcribe, refine, translate, and export.</span
          ></button
        ><button onclick={() => chooseKind('voiceover')} class="choice"
      ...
      <button
        onclick={() => chooseKind(kind, true)}
        class="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-[var(--line)] px-4 py-3 text-sm font-semibold"
        ><Gauge size={17} /> Skip guidance and open a fully customizable workspace</button
      >
    {:else if step === 2}
      <div class="mt-7 grid gap-3 sm:grid-cols-5">
        {#each [{ id: 'upload', label: 'Upload', icon: Upload }, { id: 'paste', label: 'Paste', icon: FileText }, { id: 'url', label: 'URL', icon: Link2 }, { id: 'reuse', label: 'Reuse', icon: BookOpenText }, { id: 'later', label: 'Add later', icon: Gauge }] as mode}{@const Icon =
            ...
            class="source-choice"><Icon size={18} />{mode.label}</button
          >{/each}
      </div>
      <div class="mt-5 rounded-2xl border border-[var(--line)] p-5">
        ...
        {:else if sourceMode === 'paste'}
          <textarea
            ...
            class="w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] p-4"
          ></textarea>
          <div class="mt-2">
            <SearchReplaceBar
              texts={[pastedText]}
    ...
    {:else if step === 3}
      {#if kind === 'audiobook'}
        <div class="mt-7 grid gap-4 md:grid-cols-2">
          <label class="option"
            ><input type="checkbox" bind:checked={normalizeText} /><span
              ><strong>Normalize text for speech</strong><small
                >Deterministically expands and cleans text patterns that speech
                engines commonly misread. No provider cost.</small
              ></span
            ></label
          >
          <label class="option"
            ><input type="checkbox" bind:checked={optimizeTts} /><span
              ><strong>Optimize each segment with an LLM</strong><small
                >Optional pronunciation and numeral cleanup immediately before
                synthesis. Local models can run without API charges; remote
                providers may incur cost.</small
              ></span
            ></label
          >
          <label class="text-sm font-semibold"
            >Narration language<select
          ...
          >
          <label class="text-sm font-semibold"
            >Audiobook format<select
              bind:value={audiobookFormat}
              class="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-4 py-3"
              ><option value="m4b">M4B · chapters, metadata, and cover</option
              ><option value="mp3">MP3</option><option value="opus">Opus</option
              ><option value="flac">FLAC</option><option value="wav"
                >PCM WAV</option
              ></select
            ></label
          >
          <div class="rounded-2xl bg-[var(--accent-soft)] p-4 text-sm">
            <strong>Document cleaning stays reviewable</strong>
            <p class="muted mt-1 text-xs leading-relaxed">
              Pandrator extracts and cleans the source first. Headings become
              editable chapter starts in the generation drawer; the original
              file is preserved.
            </p>
          </div>
        </div>
        ...
        <div class="mt-7 grid gap-4 md:grid-cols-2">
          ...
          >{#if translate}<label class="text-sm font-semibold"
            ...
            >{/if}{#if kind === 'subtitles'}<label class="text-sm font-semibold"
              >Export as<select
                bind:value={subtitleExport}
                class="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-4 py-3"
                ><option value="srt">SubRip (.srt)</option><option value="vtt"
                  >WebVTT (.vtt)</option
                ><option value="text">Concatenated plain text (.txt)</option
                ></select
              ><small class="muted mt-2 block font-normal"
                >This workspace exports subtitle documents, not rendered video.
                You can convert it to a voiceover workspace later.</small
              ></label
            >{:else}<label class="text-sm font-semibold"
              >Video subtitles<select
                bind:value={subtitleMode}
                class="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-4 py-3"
                ><option value="none">No subtitle track</option><option
                  value="soft">Soft / selectable subtitles</option
                ><option value="burned">Burn into video</option></select
              ></label
            >{/if}
        </div>
      {/if}
      ...
      <div class="mt-7 grid gap-6 md:grid-cols-[1fr_1.2fr]">
        <div>
          ...
          <p class="muted mt-3 text-sm">
            This only controls the display name; storage uses a stable UUID.
          </p>
        </div>
        <div
        ...
        >
          ...
          <div class="mt-4 flex flex-wrap items-center gap-2">
            {#each pipeline as stage, index}<span
                ...
                >{stage}</span
              >{#if index < pipeline.length - 1}<ArrowRight
                  class="muted"
                  size={15}
                />{/if}{/each}
          </div>
      ...
      {#if creating}<div class="mt-5">
          <div class="mb-1.5 flex items-center justify-between gap-3 text-xs">
            <span class="muted truncate">{progressDetail}</span><span
              class="muted shrink-0 tabular-nums"
    ...
    {/if}
    {#if step > 1}<footer class="mt-8 flex items-center justify-between">
        <button
          onclick={() => (step -= 1)}
          disabled={creating}
          class="flex items-center gap-2 rounded-xl border border-[var(--line)] px-4 py-2.5 text-sm font-semibold"
          ><ArrowLeft size={16} />
          {step === 2 && startAtSource ? 'Change task' : 'Back'}</button
        >{#if step === 2}<button onclick={nextFromSource} class="primary"
            >Continue <ArrowRight size={16} /></button
          >{:else if step === 3}<button
            onclick={() => {
              inferName();
              step = 4;
            }}
