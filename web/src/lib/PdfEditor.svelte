<script lang="ts">
  ...
  let error = $state('');
  let mode = $state<'all' | 'left' | 'right' | 'single'>('all');
  ...
  function pointerUp(event: PointerEvent) {
    ...
    const targets = mode === 'single' ? [activeInfo] : metadata.pages.filter((page) => mode === 'all' || page.side === mode);
    pushUndo();
    for (const page of targets) {
      ...
      if (tool === 'crop') {
        ...
        plan.whiteouts = [...plan.whiteouts, { original_page: page.original_page, rect, color: [1, 1, 1] }];
      }
    }
