...
<div
  class="fixed inset-0 z-[75] grid place-items-center bg-black/45 p-3 backdrop-blur-sm"
  role="presentation"
  onclick={(event) => event.target === event.currentTarget && onclose()}
>
  <div
    use:modalFocus={{ onclose }}
    class="surface flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl"
    role="dialog"
    aria-modal="true"
  ...
  >
    ...
    >
      <div>
        ...
        {#if description}<p class="muted mt-2 max-w-3xl text-sm">
            {description}
          </p>{/if}
      </div>
      <button
        ...
        aria-label="Close settings"><X size={20} /></button
      >
    </header>
    <div class="modal-scroll min-h-0 flex-1 bg-[var(--paper)] p-5 sm:p-7">
      {#if sessionId && section === 'output'}<OutputSettingsPanel
          {sessionId}
        />{:else if sessionId}<SettingsPanel
          {sessionId}
          {section}
          {title}
          {initialOverride}
