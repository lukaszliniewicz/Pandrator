<script lang="ts">
  ...
  async function save(item: CredentialProfile) {
    ...
    if (['database', 'keyring'].includes(backend) && !apiKey && !(item.credential_configured && backend === item.credential_backend)) {
      error = `Enter a ${item.label} API key.`; return;
    }
    ...
    try {
      ...
      notice = updated.previous_credential_retained && backend !== item.credential_backend
...
<section>
  <header><div class="eyebrow">Other API keys</div><h2 class="mt-1 text-2xl font-semibold">Supporting services</h2><p class="muted mt-2 max-w-2xl text-sm">Keys used outside the LLM and speech provider catalogues are managed here with the same write-only behavior. Direct database entry remains the default and requires no operating-system setup.</p></header>
  ...
  <div class="mt-5 grid gap-4 lg:grid-cols-2">
    {#each items as item}
      <article class="rounded-2xl border border-[var(--line)] p-5">
        ...
        <div class="mt-5"><CredentialStorageFields backends={credentialBackends} bind:backend={selectedBackends[item.id]} bind:reference={references[item.id]} bind:secret={values[item.id]} bind:deletePrevious={deletePrevious[item.id]} configured={item.credential_configured} currentSource={item.credential_source} existingBackend={item.credential_backend} suggestedEnvironment={item.environment_variable}/></div>
        <div class="mt-4 flex flex-wrap gap-2"><button onclick={() => save(item)} disabled={busy === item.id} class="btn btn-primary">{selectedBackends[item.id] !== item.credential_backend ? 'Verify move' : item.credential_configured ? 'Update credential' : 'Save credential'}</button>{#if item.credential_configured}<button onclick={() => remove(item)} disabled={busy === item.id} class="btn btn-secondary text-red-500"><Trash2 size={14}/> Remove credential</button>{/if}</div>
      </article>
    {/each}
  </div>
</section>
