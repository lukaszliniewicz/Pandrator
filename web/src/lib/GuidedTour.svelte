<script lang="ts">
  ...
  let {
    ...
    steps,
    open = $bindable(false)
  }: { tourId: string; steps: Step[]; open?: boolean } = $props();
  let index = $state(0);
  const current = $derived(steps[index]);
  function close(completed = false) {
    if (completed) localStorage.setItem(`pandrator-tour-${tourId}`, 'complete');
    index = 0;
    open = false;
  }
  function next() {
...
{#if open && current}
  ...
  >
    <div
      ...
      class="surface fixed right-5 bottom-5 w-[min(27rem,calc(100vw-2.5rem))] rounded-3xl p-6 shadow-2xl"
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      <div class="mt-5 flex items-center gap-1">
        {#each steps as _, step}<span
            class:active={step === index}
            class="h-1.5 flex-1 rounded-full bg-[var(--line)]"
          ></span>{/each}
      </div>
      <div class="mt-5 flex justify-between">
        <button
          ...
          disabled={index === 0}
          class="flex items-center gap-2 rounded-xl border border-[var(--line)] px-3 py-2 text-sm font-semibold disabled:opacity-30"
          ><ArrowLeft size={15} /> Back</button
        ><button
          onclick={next}
          class="flex items-center gap-2 rounded-xl bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white"
          >{index + 1 === steps.length
            ? 'Finish'
            : 'Next'}{#if index + 1 === steps.length}<Check
              size={15}
            />{:else}<ArrowRight size={15} />{/if}</button
        >
      </div>
    </div>
  </div>
