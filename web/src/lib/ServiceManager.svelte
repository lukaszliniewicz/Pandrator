<script lang="ts">
  ...
  async function persistEditing() {
    if (!editing) return;
    const record = recordFrom({ ...editing, models: modelChoices, default_model: editing.default_model || selectedModel }, editing);
  ...
  async function saveDiscovered() {
    ...
    discoverOpen = false; result = null;
  }
