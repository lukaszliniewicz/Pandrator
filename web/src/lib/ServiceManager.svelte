<script lang="ts">
  ...
  async function persistEditing() {
    if (!editing) return;
    const record = { ...recordFrom({ ...editing, models: modelChoices, default_model: editing.default_model || selectedModel }, editing), ...(editingApiKey.trim() ? { api_key: editingApiKey.trim() } : {}), ...(removeEditingApiKey ? { clear_api_key: true } : {}) };
