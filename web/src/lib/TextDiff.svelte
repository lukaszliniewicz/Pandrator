<script lang="ts">
  ...
  let {
    ...
    after = '',
    view = 'combined'
  }: { before: string; after: string; view?: DiffView } = $props();
  ...
  function lcsDiff(left: string[], right: string[]): DiffPart[] {
    ...
    while (i < left.length || j < right.length) {
      if (i < left.length && j < right.length && left[i] === right[j]) {
        ...
        parts.push({ kind: 'remove', text: left[i] });
        i += 1;
      }
    }
  ...
  function buildDiff(left: string, right: string) {
    ...
    return leftTokens.length * rightTokens.length <= 250_000
      ? lcsDiff(leftTokens, rightTokens)
...
<div
  ...
  aria-label={view === 'before'
    ? 'Original text with removals highlighted'
    : view === 'after'
      ? 'Corrected text with additions highlighted'
      : 'Text differences'}
>
  {#each parts.filter((part) => view === 'combined' || part.kind === 'same' || (view === 'before' && part.kind === 'remove') || (view === 'after' && part.kind === 'add')) as part}<span
      class:add={part.kind === 'add'}
      class:remove={part.kind === 'remove'}>{part.text}</span
    >{/each}
</div>
...
<style>
  ...
  .add {
    ...
    color: color-mix(in srgb, #15803d 85%, var(--ink));
    text-decoration: none;
  }
  .remove {
    border-radius: 0.2rem;
    background: color-mix(in srgb, #ef4444 20%, transparent);
    color: color-mix(in srgb, #b91c1c 85%, var(--ink));
