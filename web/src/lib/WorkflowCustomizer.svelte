<script lang="ts">
  ...
  let {
    ...
    onclose,
    onsaved
  }: { sessionId: string; onclose: () => void; onsaved: () => void } = $props();
  ...
  let originalWorkflowKind =
    $state<SessionRecord['workflow_kind']>('audiobook');
  let error = $state('');
  let saving = $state(false);
  const transformations = $derived(payload?.value?.transformations ?? {});
  const deliverables = $derived(payload?.value?.deliverables ?? {});
  const speechOptimizationEnabled = $derived(
    Boolean(
      transformations.llm_tts_document_optimization ||
      transformations.llm_tts_optimization
    )
  );
  const speechOptimizationTiming = $derived(
    transformations.llm_tts_document_optimization ? 'document' : 'generation'
  );

  function setSpeechOptimizationEnabled(enabled: boolean) {
    transformations.llm_tts_document_optimization =
      enabled && speechOptimizationTiming === 'document';
    transformations.llm_tts_optimization =
      enabled && speechOptimizationTiming === 'generation';
  }

  function setSpeechOptimizationTiming(timing: string) {
    transformations.llm_tts_document_optimization =
      speechOptimizationEnabled && timing === 'document';
    transformations.llm_tts_optimization =
      speechOptimizationEnabled && timing === 'generation';
  }
  ...
  function setTransformation(key: string, enabled: boolean) {
    ...
    if (key === 'translate') {
      if (enabled && transformations.generate_audio)
        current.value.inputs.generation = 'translation';
      if (!enabled && current.value.inputs.generation === 'translation')
        current.value.inputs.generation = transformations.correct
          ? 'correction'
          : 'source';
    }
    if (key === 'generate_audio' && enabled && transformations.translate)
      current.value.inputs.generation = 'translation';
    if (key === 'correct' && !enabled) {
      if (current.value.inputs.translation === 'correction')
        current.value.inputs.translation = 'source';
      if (current.value.inputs.generation === 'correction')
        current.value.inputs.generation = transformations.translate
          ? 'translation'
          : 'source';
    }
  }
  ...
  function setWorkflowKind(kind: 'subtitles' | 'voiceover') {
    ...
    if (kind === 'subtitles') {
      ...
      if (transformations.translate)
        payload.value.inputs!.generation = 'translation';
  ...
  async function load() {
    ...
    payload.value.workflow_kind =
      payload.value.workflow_kind ?? session.workflow_kind;
  ...
  async function save() {
    ...
    try {
      if (session)
        await sessionApi.update(sessionId, session.revision, {
          source_language: sourceLanguage,
          target_language: transformations.translate ? targetLanguage : null
        });
      const selectedWorkflowKind = String(
        current.value.workflow_kind ?? originalWorkflowKind
      );
      if (
        selectedWorkflowKind !== originalWorkflowKind &&
        ['subtitles', 'voiceover'].includes(selectedWorkflowKind)
      ) {
        const output = await sessionApi.settings(sessionId, 'output');
        const nextOutput =
          selectedWorkflowKind === 'subtitles'
            ? {
                ...output.override,
            ...
            : {
                ...
                audio_mode: 'mixed',
                subtitle_mode:
                  output.override?.subtitle_mode === 'burned'
                    ? 'burned'
                    : 'soft'
              };
      ...
      await sessionApi.updateOutcome(sessionId, current.revision, {
        ...current.value,
        export: {
...
<div class="fixed inset-0 z-[70] grid place-items-center bg-black/40 p-5">
  <div
    ...
    class="surface max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-3xl p-7"
    role="dialog"
    aria-modal="true"
    aria-labelledby="workflow-customizer-title"
  >
    <header class="flex justify-between">
      ...
      <button onclick={onclose} aria-label="Close workflow customizer"
        ><X size={19} /></button
      >
    </header>
    ...
    {#if payload}
      {#if session?.workflow_kind !== 'audiobook'}
        <div class="mt-6 rounded-2xl border border-[var(--line)] p-5">
          ...
          <div class="mt-3 grid gap-3 sm:grid-cols-2">
            <button
              type="button"
              onclick={() => setWorkflowKind('subtitles')}
              class:active={payload.value.workflow_kind === 'subtitles'}
              class="workspace-kind"
              ><strong>Subtitles</strong><span
                >SRT, VTT, or concatenated text; no voice or video controls.</span
              ></button
            ><button
              type="button"
              onclick={() => setWorkflowKind('voiceover')}
              class:active={payload.value.workflow_kind === 'voiceover'}
              class="workspace-kind"
              ><strong>Voiceover</strong><span
                >Add speech generation, audio review, and rendered media
                exports.</span
              ></button
            >
          </div>
      ...
      <div class="mt-6 grid gap-3 sm:grid-cols-2">
        <label class="text-sm font-semibold"
          >Source language<select bind:value={sourceLanguage} class="field"
            >{#each LANGUAGE_OPTIONS as item}<option value={item.value}
                >{item.label}</option
              >{/each}</select
          ></label
        >
        {#if transformations.translate}<label class="text-sm font-semibold"
            >Target language<select bind:value={targetLanguage} class="field"
        ...
        <div class="rounded-2xl border border-[var(--line)] p-5">
          ...
          <div class="mt-4 space-y-3">
            {#each [{ key: 'transcribe', label: 'Transcribe media' }, { key: 'correct', label: 'Correct same-language subtitles' }, { key: 'translate', label: 'Translate' }, { key: 'deterministic_normalization', label: 'Deterministic speech normalization' }, { key: 'generate_audio', label: 'Generate speech' }, { key: 'rvc', label: 'Create RVC variants' }].filter((item) => payload?.value.workflow_kind !== 'subtitles' || !['deterministic_normalization', 'generate_audio', 'rvc'].includes(item.key)) as item}<label
                ...
                ><input
                  type="checkbox"
                  checked={Boolean(transformations[item.key])}
                  onchange={(event) =>
                    setTransformation(item.key, event.currentTarget.checked)}
                  class="mt-1 accent-[var(--accent)]"
                /><span>{item.label}</span></label
              >{/each}{#if payload.value.workflow_kind !== 'subtitles'}<div
                class="rounded-xl border border-[var(--line)] p-3"
              >
                <label class="flex items-start gap-3 text-sm"
                  ><input
                    type="checkbox"
                    checked={speechOptimizationEnabled}
                    onchange={(event) =>
                      setSpeechOptimizationEnabled(event.currentTarget.checked)}
                    class="mt-1 accent-[var(--accent)]"
                  /><span>Optimize text for speech</span></label
                >{#if speechOptimizationEnabled}<select
                    value={speechOptimizationTiming}
                    onchange={(event) =>
          ...
          <div class="mt-4 rounded-xl bg-[var(--accent-soft)] p-3 text-sm">
            <strong
              >{payload.value.workflow_kind === 'audiobook'
                ? 'Audiobook audio'
                : payload.value.workflow_kind === 'voiceover'
                  ? 'Voiceover / dubbed media'
                  : 'SRT, VTT, or plain text'}</strong
            >
            <p class="muted mt-1 text-xs">
              This primary deliverable follows the selected workspace type.
            </p>
          </div>
          {#if payload.value.workflow_kind === 'voiceover'}<label
              class="mt-4 flex items-start gap-3 text-sm"
              ><input
                type="checkbox"
                bind:checked={deliverables.subtitles}
                class="mt-1 accent-[var(--accent)]"
              /><span>Also include subtitle files or tracks</span></label
            >{/if}{#if transformations.translate}<label
              class="mt-5 block text-sm font-semibold"
              >Translation input<select
                bind:value={payload.value.inputs.translation}
                class="field"
                ><option value="source">Source / transcription directly</option
                ><option value="correction" disabled={!transformations.correct}
                  >Corrected subtitles</option
                ></select
              ></label
            >{/if}{#if transformations.generate_audio}<label
              class="mt-4 block text-sm font-semibold"
              >Generation input<select
                bind:value={payload.value.inputs.generation}
                class="field"
                ><option value="source">Source text/subtitles</option><option
                  value="correction"
                  disabled={!transformations.correct}>Correction</option
                ><option
                  value="translation"
                  disabled={!transformations.translate}>Translation</option
                ></select
              ></label
            >{/if}
        </div>
      ...
      <div class="mt-5 rounded-2xl bg-[var(--accent-soft)] p-5">
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2">
          {#each payload.pipeline as stage, index}<span
              ...
              >{stage.title}</span
            >{#if index < payload.pipeline.length - 1}<ArrowRight
                size={15}
              />{/if}{/each}
        </div>
      </div>
