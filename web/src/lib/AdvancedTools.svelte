...
<div class="flex min-h-[calc(100vh-5rem)] flex-col">
  <header class="mb-7 flex flex-wrap items-start justify-between gap-4"><div><button onclick={onback} class="muted mb-3 flex items-center gap-2 text-sm"><ArrowLeft size={15}/> Workspace</button><div class="eyebrow">Speech laboratory</div><h1 class="mt-2 text-3xl font-semibold">{mode==='rvc'?'RVC conversion':mode==='training'?'XTTS fine-tuning':'RVC conversion and XTTS fine-tuning'}</h1><p class="muted mt-2 max-w-3xl">{mode==='rvc'?'Manage paired model weights and indexes, then create non-destructive speech-to-speech variants from generated or imported audio.':mode==='training'?'Prepare datasets and train XTTS voices through recoverable background jobs.':'Install conversion models, create derived voice variants, and train XTTS models through durable jobs.'}</p></div><button onclick={load} class="rounded-xl border border-[var(--line)] p-2.5" aria-label="Refresh"><RefreshCw size={17}/></button></header>
  ...
  <div class="grid gap-5 xl:grid-cols-2">
    ...
    <section class:hidden={mode==='rvc'} class="surface rounded-3xl p-6"><div class="flex items-center gap-3"><div class="grid size-11 place-items-center rounded-2xl bg-[var(--accent-soft)] text-[var(--accent)]"><BrainCircuit size={21}/></div><div><h2 class="text-xl font-semibold">XTTS fine-tuning</h2><p class="muted text-sm">Long-running, recoverable worker job</p></div></div>
      ...
      <div class="mt-6 space-y-2">{#each training.slice(0,6) as item}<div class="flex items-center gap-3 rounded-xl border border-[var(--line)] px-3 py-2.5"><span class="size-2 rounded-full" class:bg-green-500={item.status==='succeeded'} class:bg-amber-500={['queued','running','cancel_requested'].includes(item.status)} class:bg-red-500={['failed','interrupted','canceled'].includes(item.status)}></span><div class="min-w-0 flex-1"><div class="truncate text-sm font-semibold">{item.model_name}</div><div class="muted text-xs">{item.status}{item.error_message?` · ${item.error_message}`:''}</div></div>{#if ['queued','running'].includes(item.status)}<button onclick={()=>cancel(item)} class="text-xs font-semibold">Cancel</button>{:else if ['failed','interrupted','canceled'].includes(item.status)}<button onclick={()=>retry(item)} disabled={busy} class="text-xs font-semibold">Retry</button>{:else if item.output_artifact_id}<a href={`/api/v1/artifacts/${item.output_artifact_id}/content`} class="text-xs font-semibold">Manifest</a>{/if}</div>{:else}<p class="muted text-sm">No training runs yet.</p>{/each}</div>
    </section>
  </div>
</div>
