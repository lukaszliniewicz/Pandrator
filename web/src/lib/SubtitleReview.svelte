<script lang="ts">
  import { Columns3, Filter, Merge, Save, Scissors, X } from '@lucide/svelte';
  import { api } from './api';
...
<div class="fixed inset-0 z-50 bg-black/45 p-3 backdrop-blur-sm sm:p-6" role="presentation">
  <div class="surface mx-auto flex h-full max-w-[96rem] flex-col overflow-hidden rounded-[1.5rem]" role="dialog" aria-modal="true" aria-labelledby="review-title">
    ...
    {#if sourceArtifactId}<div class="border-b border-[var(--line)] px-5 py-3 sm:px-7"><audio class="h-9 w-full" controls src={`/api/v1/artifacts/${sourceArtifactId}/content`}></audio></div>{/if}
        ...
        <table class="w-full min-w-[66rem] border-collapse text-sm">
          ...
          <tbody>{#each visibleRows as row}<tr class:changed={row.changed} class="align-top"><td class="muted border-b border-r border-[var(--line)] p-3 font-mono text-xs">{(row.start_ms/1000).toFixed(2)}<br/>→ {(row.end_ms/1000).toFixed(2)}</td>
            {#each availableStages as stage}<td class="border-b border-r border-[var(--line)] p-3 last:border-r-0"><div class="space-y-3">{#each row[stage] ?? [] as segment}<div class="rounded-xl border border-[var(--line)] bg-[var(--paper)] p-2.5">
              {#if stage === editStage}<div class="mb-2 grid grid-cols-2 gap-2"><label class="muted text-[.68rem]">Start ms<input type="number" bind:value={segment.start_ms} class="mt-1 w-full rounded-lg border border-[var(--line)] bg-transparent px-2 py-1"/></label><label class="muted text-[.68rem]">End ms<input type="number" bind:value={segment.end_ms} class="mt-1 w-full rounded-lg border border-[var(--line)] bg-transparent px-2 py-1"/></label></div><textarea bind:value={segment.text} rows="3" class="w-full resize-y rounded-lg border border-[var(--line)] bg-transparent p-2 leading-relaxed"></textarea><div class="mt-2 flex gap-2"><button onclick={() => split(segment)} class="flex items-center gap-1 rounded-lg border border-[var(--line)] px-2 py-1 text-xs"><Scissors size={13}/> Split</button><button onclick={() => mergeNext(segment)} class="flex items-center gap-1 rounded-lg border border-[var(--line)] px-2 py-1 text-xs"><Merge size={13}/> Merge next</button></div>{:else}<p class="whitespace-pre-wrap leading-relaxed">{segment.text}</p>{/if}
