...
<div class="mx-auto max-w-5xl">
  ...
  <div class="mt-8 grid gap-5 md:grid-cols-2">
    ...
    <section class="surface rounded-2xl p-6"><div class="eyebrow">Runtime</div><h2 class="mt-2 text-xl font-semibold">Capability snapshot</h2><div class="muted mt-4 space-y-2 text-sm"><div>FFmpeg: {appState.capabilities?.ffmpeg?.available ? 'Ready' : 'Missing'}</div><div>CrispASR: {appState.capabilities?.stt?.crispasr ? 'Ready' : 'Missing'}</div><div>GPU: {appState.capabilities?.gpu?.available ? `${gpuDevices.length} detected` : 'Not detected'}</div>{#each gpuDevices as device}<div class="ml-3 rounded-lg border border-[var(--line)] px-3 py-2"><strong class="text-[var(--ink)]">{device.name}</strong><span class="mt-0.5 block text-xs">{device.vendor} · {gpuMemory(device)}{device.apis?.length ? ` · ${device.apis.join(', ')}` : ''}</span></div>{/each}{#if appState.capabilities?.gpu?.guidance}<p class="text-xs leading-relaxed">{appState.capabilities.gpu.guidance}</p>{/if}</div><button onclick={() => appState.refreshCapabilities()} class="btn btn-secondary mt-5"><RefreshCw size={15}/> Probe again</button></section>
