<script lang="ts">
  ...
  function start(kind: typeof initialKind) {
    initialKind = kind;
    skipKindStep = true;
    wizard = true;
  }
  ...
  onMount(async () => {
    ...
    try {
      const setting = await settingApi.get<WebPreferences>('web.preferences');
      if (setting.value?.show_startup_wizard) {
        wizard = true;
        sessionStorage.setItem('pandrator-guided-creation-shown', '1');
      }
    } catch {
      /* Startup guidance is optional when preferences are unavailable. */
    }
  });
</script>
...
<div class="mx-auto max-w-7xl">
  ...
  <section class="mt-9 grid gap-4 md:grid-cols-3">
    <button onclick={() => start('audiobook')} class="task"
      ...
      <p>Clean, prepare, narrate, review, and export long-form text.</p></button
    ><button onclick={() => start('subtitles')} class="task"
      ><Captions size={26} />
      <h2>Create subtitles</h2>
      <p>Transcribe media or refine and translate an existing SRT.</p></button
    ><button onclick={() => start('voiceover')} class="task"
      ><AudioLines size={26} />
    ...
    <section>
      <div class="mb-3 flex items-center justify-between">
        ...
        <a
          ...
          >View all <ChevronRight size={14} /></a
        >
      </div>
      ...
      <div class="surface rounded-2xl p-5">
        <div class="space-y-4">
          ...
          <div class="readiness">
            <Activity size={18} />
            <div>
              <strong>Background work</strong><span
                >{appState.jobs.filter((job) =>
                  ['queued', 'running'].includes(job.status)
                ).length} active or queued jobs</span
              >
            </div>
          </div>
          <div class="readiness">
        ...
        <a
          ...
          onclick={() =>
            appState.showSetupReturn(
              'Configure providers and services, then return to finish setup.'
            )}
          class="mt-5 block rounded-xl border border-[var(--line)] px-4 py-2.5 text-center text-sm font-semibold"
          >Review setup</a
        >
      </div>
    </aside>
...
{#if wizard}<NewSessionWizard
    {initialKind}
    startAtSource={skipKindStep}
