<script lang="ts">
  ...
  function start(kind: typeof initialKind) { initialKind=kind;wizard=true; }
...
{#if wizard}<NewSessionWizard initialKind={initialKind} onclose={()=>wizard=false}/>{/if}
