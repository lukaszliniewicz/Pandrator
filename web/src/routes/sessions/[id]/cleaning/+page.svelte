<script lang="ts">
  ...
  async function load() {
    sources = (await sessionApi.sources(sessionId)).items.filter((item) =>
      ...
      <div class="flex justify-between">
        <div>
          ...
          <p class="muted mt-2 text-sm">
            Phase summaries, operations, validation warnings, tool results, and
            costs. Chain-of-thought is never stored.
          </p>
        </div>
