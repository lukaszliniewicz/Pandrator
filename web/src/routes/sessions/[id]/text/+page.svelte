...
<div class="space-y-5">
  <div class="flex flex-wrap items-end justify-between gap-4">
    <div>
      ...
      <p class="muted mt-2">
        Correction creates a same-language asset; translation and TTS
        optimization remain independent transformations.
      </p>
    </div>
    <button
      onclick={() => (review = true)}
      ...
      ><Columns3 size={16} /> Compare subtitles</button
    >
  </div>

  <nav
    class="inline-flex rounded-xl border border-[var(--line)] bg-[var(--paper-strong)] p-1"
    aria-label="Text workspace sections"
  >
    <button
      onclick={() => (activeTab = 'settings')}
      class:active={activeTab === 'settings'}
      ><SlidersHorizontal size={15} /> Settings</button
    >
    <button
      onclick={() => (activeTab = 'history')}
      class:active={activeTab === 'history'}
      ><History size={15} /> Document history
  ...
  {:else}
    <section class="surface rounded-2xl p-5">
      ...
      <div class="mt-4 space-y-2">
            ...
            <summary>
              ...
              <span class="min-w-0 flex-1"
                ><strong class="capitalize"
                  >{document.stage.replaceAll('_', ' ')}</strong
                ><small
                  >{document.language ?? 'Language unspecified'} · {document
                    .revisions.length} revision(s)</small
                ></span
              >
              <span class="muted hidden text-right text-xs sm:block"
                ><Clock3 class="inline" size={13} />
                {new Date(
                  document.revisions[0]?.created_at ?? document.created_at
                ).toLocaleString()}</span
              >
            </summary>
                ...
                <article>
                  <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                      ...
                      >{#if revision.id === document.active_revision_id}<span
                          class="badge">Current</span
                        >{/if}{#if revision.reviewed}<span
                          class="badge reviewed">Reviewed</span
                        >{/if}
                    </div>
                    <div class="muted mt-1 flex flex-wrap gap-x-2 text-xs">
                      <time datetime={revision.created_at}
                        >{new Date(revision.created_at).toLocaleString()}</time
                      ><span>· {revision.segment_count} segment(s)</span
                      >{#if revision.duration_ms}<span
                          >· {duration(revision.duration_ms)}</span
                        >{/if}{#if revision.artifact?.size_bytes != null}<span
                          >· {formatBytes(revision.artifact.size_bytes)}</span
                        >{/if}
                    </div>
                    {#if revision.artifact}<div
                        class="muted mt-1 truncate text-xs"
                      >
                        {artifactFilename(revision.artifact)} · {revision
                  ...
                  {#if revision.artifact}<div class="flex shrink-0 gap-1">
                      <button
                        ...
                        ><Eye size={15} /></button
                      ><a
                        href={`/api/v1/artifacts/${revision.artifact.id}/content`}
                        download={artifactFilename(revision.artifact)}
                        class="tool"
                        title="Download revision"
                        aria-label={`Download revision ${revision.revision_number}`}
                        ><Download size={15} /></a
                      >
                    </div>{/if}
                </article>
        ...
        {:else}
          <p class="muted py-8 text-center text-sm">
            Process a source to create the first document revision.
          </p>
        {/each}
      </div>
    </section>
...
<style>
  ...
  .revision-list article:last-child {
    border-bottom: 0;
  }
  .badge {
    border-radius: 999px;
