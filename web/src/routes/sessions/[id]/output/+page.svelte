<script lang="ts">
  ...
  let artifacts=$state<any[]>([]); let runs=$state<any[]>([]); let session=$state<any>(null); let outputProfile=$state<any>(null); let selectedRunId=$state(''); let busy=$state(false); let message=$state(''); let error=$state(''); let preview=$state<any|null>(null);
  async function load(){
    const [artifactPayload,runPayload,sessionPayload,settingsPayload]=await Promise.all([api<{items:any[]}>(`/artifacts?session_id=${sessionId}&limit=300`),api<{items:any[]}>(`/sessions/${sessionId}/generation-runs`),api<any>(`/sessions/${sessionId}`),api<any>(`/sessions/${sessionId}/settings/output`)]);
