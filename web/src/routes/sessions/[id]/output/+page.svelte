<script lang="ts">
  ...
  import { artifactApi, generationApi, jobApi, sessionApi } from '$lib/domain-api';
  ...
  const hasSourceVideo=$derived(Boolean(outputProfile?.context&&typeof outputProfile.context==='object'&&(outputProfile.context as Record<string,unknown>).has_source_video));
  type SaveOutputProfile=()=>Promise<{output:Record<string,unknown>;audio:Record<string,unknown>}>;
  let saveOutputProfile=$state<SaveOutputProfile|null>(null);
  async function load(){
    const [artifactPayload,runPayload,sessionPayload,settingsPayload,jobPayload]=await Promise.all([artifactApi.list({sessionId,limit:300}),generationApi.runs(sessionId),sessionApi.get(sessionId),sessionApi.settings(sessionId,'output'),jobApi.list(500)]);
    artifacts=artifactPayload.items.filter((item)=>item.role==='export'||item.role.startsWith('export_')||['assembled_audio','audiobook_audio','dubbing_audio','output_assembly','rvc_audio'].includes(item.role)).sort((left,right)=>String(right.created_at).localeCompare(String(left.created_at)));
    ...
    if(!selectedRunId||!runs.some((item)=>item.id===selectedRunId))selectedRunId=runs.find((item)=>item.status==='completed')?.id??'';
  }
  ...
  async function assemble(){
    ...
    try{
      ...
      const resolvedAssemblySettings=needsAssembly?await sessionApi.resolveSettings(sessionId,['audio','output']):null;
      ...
      if(needsAssembly&&!assemblyIsCurrent){
        ...
        if(!assemblyMatchesSettings||!['queued','running'].includes(selected?.assembly?.status??''))await generationApi.createAssembly(sessionId,selectedRunId);
        await waitForAssembly(selectedRunId);
      }
      const job=await sessionApi.runStage(sessionId,'export',needsAssembly?{generation_run_id:selectedRunId}:{});
  ...
  function canRemove(artifact:ArtifactRecord){return artifact.kind==='export'||artifact.role==='export'||artifact.role.startsWith('export_')}
  async function removeExport(artifact:ArtifactRecord){
    ...
    try{await sessionApi.removeOutput(sessionId,artifact.id);artifacts=artifacts.filter((item)=>item.id!==artifact.id);if(preview?.id===artifact.id)preview=null;message='Export removed.'}
...
<div class="space-y-5">
  <div class="flex flex-wrap items-end justify-between gap-4"><div><h2 class="text-2xl font-semibold">{session?.workflow_kind==='subtitles'?'Export subtitles':session?.workflow_kind==='audiobook'?'Audiobook output':hasSourceVideo?'Video output':'Voiceover output'}</h2><p class="muted mt-2">{session?.workflow_kind==='subtitles'?'Save the selected subtitle document as SRT, WebVTT, or concatenated plain text.':session?.workflow_kind==='audiobook'?'Assemble the selected narration takes with book metadata, chapters, and optional cover artwork.':hasSourceVideo?'Create a mixed, source-only, or voiceover-only video with optional subtitle tracks.':'Create standalone voiceover audio plus optional subtitle or text documents.'}</p></div><div class="flex flex-wrap items-end gap-2">{#if runs.length&&session?.workflow_kind!=='subtitles'}<label class="text-xs font-semibold">Audio version<select bind:value={selectedRunId} class="mt-1 block max-w-sm rounded-xl border border-[var(--line)] bg-[var(--paper-strong)] px-3 py-2 text-sm font-normal"><option value="">Do not select generated audio</option>{#each runs.filter((item)=>item.status==='completed') as item}<option value={item.id}>{item.label}</option>{/each}</select></label>{/if}<button onclick={assemble} disabled={busy||!saveOutputProfile} class="flex items-center gap-2 rounded-xl bg-[var(--accent)] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{#if busy}<LoaderCircle class="animate-spin" size={16}/>{:else}<PackageCheck size={16}/>{/if} {session?.workflow_kind==='subtitles'?'Create subtitle export':'Create export'}</button></div></div>
  ...
  {#if exportJobs.length}<section class="surface rounded-2xl p-5"><div class="eyebrow">Export activity</div><div class="mt-4 space-y-3">{#each exportJobs.slice(0,4) as job (job.id)}<div class="flex flex-wrap items-center gap-3 rounded-xl border border-[var(--line)] p-3">{#if ['queued','running','cancel_requested'].includes(job.status)}<LoaderCircle class="animate-spin text-[var(--accent)]" size={18}/>{:else if job.status==='succeeded'}<CheckCircle2 class="text-[var(--success)]" size={18}/>{:else}<CircleAlert class="text-red-500" size={18}/>{/if}<div class="min-w-0 flex-1"><div class="font-semibold">{jobLabel(job)} export <span class="muted font-mono text-xs">{job.id.slice(0,8)}</span></div><div class="muted mt-1 text-xs">{progressPercent(job)}% · {new Date(job.created_at).toLocaleString()}</div>{#if progressDetail(job)&&['queued','running','cancel_requested'].includes(job.status)}<div class="muted mt-1 truncate text-xs" title={progressDetail(job)}>{progressDetail(job)}</div>{/if}{#if job.error_message}<div class="mt-1 text-xs text-red-500">{job.error_message}</div>{/if}</div><div class="h-1.5 w-32 overflow-hidden rounded-full bg-[var(--line)]" role="progressbar" aria-label={`Export ${job.id.slice(0,8)} progress`} aria-valuemin="0" aria-valuemax="100" aria-valuenow={progressPercent(job)}><div class="h-full bg-[var(--accent)] transition-[width]" style={`width:${progressPercent(job)}%`}></div></div></div>{/each}</div></section>{/if}
  <OutputSettingsPanel {sessionId} onSaveForExportReady={(save)=>{saveOutputProfile=save}}/>
