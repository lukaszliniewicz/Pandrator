<script lang="ts">
  ...
  import { CheckCircle2, CircleAlert, Download, FileAudio, FileText, FileVideo, LoaderCircle, PackageCheck, Trash2 } from '@lucide/svelte';
...
<div class="space-y-5">
  ...
  <section class="surface rounded-2xl p-5"><div class="eyebrow">Completed outputs</div><div class="mt-4 grid gap-3 md:grid-cols-2">
    {#each artifacts as artifact}<article class="flex items-center gap-2 rounded-xl border border-[var(--line)] p-2"><button onclick={()=>{preview=artifact}} class="flex min-w-0 flex-1 items-center gap-3 rounded-lg p-2 text-left">{#if String(artifact.mime_type??'').startsWith('video/')||String(artifact.relative_path??'').endsWith('.mp4')}<FileVideo size={18}/>{:else if String(artifact.mime_type??'').startsWith('audio/')}<FileAudio size={18}/>{:else}<FileText size={18}/>{/if}<div class="min-w-0 flex-1"><div class="truncate font-semibold">{artifactRoleLabel(artifact.role)}</div><div class="muted truncate text-xs">{artifact.relative_path}</div></div><Download size={16}/></button>{#if canRemove(artifact)}<button onclick={()=>removeExport(artifact)} disabled={deleting[artifact.id]} aria-label={`Remove export ${artifact.relative_path.split('/').at(-1)??''}`} title="Remove export" class="rounded-lg p-2 text-red-500 hover:bg-red-500/10 disabled:opacity-50">{#if deleting[artifact.id]}<LoaderCircle class="animate-spin" size={16}/>{:else}<Trash2 size={16}/>{/if}</button>{/if}</article>{:else}<p class="muted text-sm">No completed outputs yet.</p>{/each}
  </div></section>
</div>
